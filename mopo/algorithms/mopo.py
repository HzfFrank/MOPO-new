...
class MOPO(RLAlgorithm):
    ...
    def _train(self):
        ...
        model_train_metrics = self._train_model(batch_size=256, max_epochs=max_epochs, holdout_ratio=0.2,
    ...
    def _rollout_model(self, rollout_batch_size, **kwargs):
        ...
        """
        env_samples = self._pool.return_all_samples()
        obs_list = env_samples['observations']
        act_list = env_samples['actions']  # 这里选取的action都是和obs一一对应的，都是一个sample里面出来的，而MOPO原本选择的action的根据策略来sample的
        next_obs_list = env_samples['next_observations']
        rew_list = env_samples['rewards']
        ...
        if self._model_type != 'identity':
            for i in obs_list:
                _obs = obs_list[i]
                _act = act_list[i]
                _next_obs, _rew, _term, _info = self.fake_env.step(_obs, _act, **kwargs)
                if max_rew < _rew:
                    obs = _obs
                    act = _act
                    next_obs = _next_obs
